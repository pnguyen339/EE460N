...
void latch_datapath_values() {
   ...
   */
    ...
    if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
    	int page_number = Low16bits(BUS) >> 9;

    	PTEaddr = 0;
    	PTEaddr |= MEMORY[CURRENT_LATCHES.PTBR + (page_number << 1) >> 1][1] << 8;
    	PTEaddr |= MEMORY[CURRENT_LATCHES.PTBR + (page_number << 1) >> 1][0];
    ...
    if(GetLD_CheckEXC(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        ...
        if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
			if(!(CURRENT_LATCHES.PRIV == (PTEaddr && 0x08 >> 3) ||
					CURRENT_LATCHES.TRAP_FLAG == (PTEaddr && 0x08 >> 3))) {
				Protection = 1;
			}
